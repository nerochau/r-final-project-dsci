...
```{r}
raw_bacii <- read.csv("data/bacii_result.csv")
lit_neet <- read.csv("data/literacy_neet.csv")

#convert to all numeric
new_bacii <- raw_bacii |> 
  mutate(
    total_student_registered = parse_number(total_student_registered),
...
combined_data |> 
  kable(
      ...
      caption = "Cambodia Statistics 2015-2025"
    )
```
```{r}
...
kable(
  ...
  caption = "Summary of National Exam, Literacy, and NEET Statistics (2015–2025)"
)
```
# 3 Analysis
<hr>
...
```{r warning = FALSE}
ggplot(combined_data, aes(x = year, y = pass_rate)) +
...
### Plot 2: Stacked bar of grade distribution
